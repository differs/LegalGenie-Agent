...
6. 为后续 AI 抽取提供可直接消费的原文与中文双语数据基础
...
- 阅读默认视图：双语对照
- AI 消费文本：作为后续目标，AI 将消费原文 + 中文；本轮先把数据与接口准备好
...
时间线抽取、人物抽取、证据整理、摘要生成等后续 AI 能力，长期目标是不再直接吃整份全文，而是基于 chunk 聚合执行，并消费原文与中文双语数据。
...
本轮验收不以“现有 AI 抽取链已经切换到双语输入”为条件，而是以“系统已经稳定产出并暴露可供 AI 抽取使用的双语 chunk 数据”为条件。
...
4. 搜索默认中文，可切原文和双语
5. 系统稳定产出并暴露可供 AI 抽取使用的原文与中文双语 chunk 数据
6. 翻译失败不会让文件不可用
